...
class ResnetConditionHRVeryLite_rewrite(Model):

    def __init__(self, input_nc, output_nc, \
                ngf=64, nf_part=64, \
                norm_layer=BatchNormalization, use_dropout=False, \
                n_blocks1=7, n_blocks2=3, padding_type='reflect'):
        super(ResnetConditionHRVeryLite_rewrite, self).__init__()
        ...
        # Input Layers
        self.img_input = Input(shape=(448, 448, input_nc[0]))
        self.bg_input = Input(shape=(448, 448, input_nc[1]))
        self.seg_input = Input(shape=(448, 448, input_nc[2]))

        # main encoder output 256xW/4xH/4
        model_enc1 = [ZeroPadding2D(padding=(1, 1)), \
                        Conv2D(ngf, 3, padding='valid', use_bias=use_bias), \
                        norm_layer(momentum=0.1, epsilon=1e-05), \
                        ReLU()]
        model_enc1 += [ZeroPadding2D(padding=(1, 1)), \
                        Conv2D(ngf * 2, 3, strides=2, padding='valid', use_bias=use_bias), \
                        norm_layer(momentum=0.1, epsilon=1e-05), \
                        ReLU()]

        # back encoder output 256xW/4xH/4
        model_enc_back = [ZeroPadding2D(padding=(1, 1)), \
                        Conv2D(ngf, 3, padding='valid', use_bias=use_bias), \
                        norm_layer(momentum=0.1, epsilon=1e-05), \
                        ReLU()]
        n_downsampling = 1
        for i in range(n_downsampling):
            mult = 2**i
            model_enc_back += [ZeroPadding2D(padding=(1, 1)), \
                                Conv2D(ngf * mult * 2, 3, strides=2, padding='valid', use_bias=use_bias), \
                                norm_layer(momentum=0.1, epsilon=1e-05), \
                                ReLU()]

        # seg encoder output 256xW/4xH/4
        model_enc_seg = [ZeroPadding2D(padding=(1, 1)), \
                        Conv2D(ngf, 3, padding='valid', use_bias=use_bias), \
                        norm_layer(momentum=0.1, epsilon=1e-05), \
                        ReLU()]
        n_downsampling = 1
        ...
        for i in range(n_downsampling):
            ...
            model_dec_al += [UpSampling2D(size=(2, 2), data_format=None, interpolation='bilinear'),
                            ...
                            Conv2D(int(ngf * mult / 2), 3, strides=1, padding='valid'),
                            norm_layer(momentum=0.1, epsilon=1e-05),
                            ReLU()]
        model_dec_al += [ZeroPadding2D(padding=(1, 1)),
                        Conv2D(1, 3, padding='valid'),
                        Tanh()]

        model_dec_fg1 = [UpSampling2D(size=(2, 2), data_format=None, interpolation='bilinear'),
                        ZeroPadding2D(padding=(1, 1)), \
                        Conv2D(ngf, 3, strides=1, padding='valid'),
                        norm_layer(momentum=0.1, epsilon=1e-05),
                        ReLU(),
                        ZeroPadding2D(padding=(1,1)),
    ...
    # image, back, seg
    def build(self):
        img_feat1 = self.model_enc1(self.img_input)
        img_feat = img_feat1
        
        back_feat = self.model_enc_back(self.bg_input)
        seg_feat = self.model_enc_seg(self.seg_input)
        ...
        return Model([self.img_input, self.bg_input, self.seg_input], [al_out, fg_out])
...
class ResnetBlock(Model):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        if padding_type == 'reflect':
            ...
            conv_block += [ZeroPadding2D(padding=(1, 1)), \
                        Conv2D(dim, 3, padding='valid', use_bias=use_bias)]
        else:
