...
############################## part ##################################
class ResnetConditionHRLite(nn.Module):
	def __init__(self, input_nc, output_nc, ngf=64, nf_part=64,norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks1=7, n_blocks2=3, padding_type='reflect'):
		assert(n_blocks1 >= 0); assert(n_blocks2 >= 0)
		super(ResnetConditionHRLite, self).__init__()
		self.input_nc = input_nc
		self.output_nc = output_nc
		self.ngf = ngf
		use_bias=True
		...
		#decoder
		model_res_dec=[nn.Conv2d(ngf * mult +2*nf_part,ngf*mult,kernel_size=1,stride=1,padding=0,bias=False),norm_layer(ngf*mult),nn.ReLU(True)]
		for i in range(n_blocks1):
			model_res_dec += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
		...
		# model_dec_fg1=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, int(ngf * 2), 3, stride=1,padding=1),norm_layer(int(ngf * 2)),nn.ReLU(True)]
		model_dec_fg1=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, ngf, 3, stride=1,padding=1),norm_layer(ngf),nn.ReLU(True),nn.ReflectionPad2d(3),nn.Conv2d(ngf, output_nc-1, kernel_size=7, padding=0)]

		model_dec_fg2=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, ngf, 3, stride=1,padding=1),norm_layer(ngf),nn.ReLU(True),nn.ReflectionPad2d(3),nn.Conv2d(ngf, output_nc-1, kernel_size=7, padding=0)]
	...
	def forward(self, image,back,seg):
		...
		oth_feat=torch.cat([self.comb_back(torch.cat([img_feat,back_feat],dim=1)),self.comb_seg(torch.cat([img_feat,seg_feat],dim=1))],dim=1)
...
class DWBlock(nn.Module):
    def __init__(self, in_channels, out_channels, expend=1.0):
        super(DWBlock, self).__init__()
        blocks = []
        mid_channels = int(expend * in_channels)
        blocks.append(nn.Conv2d(in_channels, mid_channels, 1, 1, 0, bias=False))
        blocks.append(nn.Conv2d(mid_channels, mid_channels, kernel_size=3, padding=1, bias=False, groups=mid_channels))
        blocks.append(nn.BatchNorm2d(mid_channels))
        if USE_RELU6:
            blocks.append(nn.ReLU6())
        else:
            blocks.append(nn.ReLU())
        blocks.append(nn.Conv2d(mid_channels, out_channels, 1, 1, 0, bias=False))
        blocks.append(nn.BatchNorm2d(out_channels))
        self.split = nn.Conv2d(in_channels, mid_channels, 1, 1, 0, bias=False)
...
class DWSplit2(nn.Module):
    def __init__(self, in_channels, out_channels, expend=1.0):
        super(DWSplit2, self).__init__()
        expend_half = expend//2
        self.split_a = DWBlock(in_channels, out_channels//2, expend=expend_half)
        self.split_b = DWBlock(in_channels, out_channels//2, expend=expend_half)
        self.pointwise = nn.Conv2d(out_channels, out_channels, 1, 1, 0, bias=False)

    def forward(self, x):
        residual = x
        xx = x
        out_a = self.split_a(x)
        out_b = self.split_b(xx)
        out = self.pointwise(torch.cat((out_a, out_b), dim=1))
        out += residual
        return out

class ResnetConditionHRVeryLite(nn.Module):
	def __init__(self, input_nc, output_nc, ngf=64, nf_part=64,norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks1=7, n_blocks2=3, padding_type='reflect'):
		assert(n_blocks1 >= 0); assert(n_blocks2 >= 0)
		...
		#main encoder output 256xW/4xH/4
		# model_enc1 = [nn.ReflectionPad2d(3),nn.Conv2d(input_nc[0], ngf, kernel_size=7, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		model_enc1 = [nn.ReflectionPad2d(1),nn.Conv2d(input_nc[0], ngf, kernel_size=3, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		model_enc1 += [nn.Conv2d(ngf , ngf * 2, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * 2),nn.ReLU(True)]
		# model_enc2 = [nn.Conv2d(ngf*2 , ngf * 4, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * 4),nn.ReLU(True)]
		

		#back encoder output 256xW/4xH/4
		model_enc_back = [nn.ReflectionPad2d(1),nn.Conv2d(input_nc[1], ngf, kernel_size=3, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		n_downsampling = 1
		for i in range(n_downsampling):
			mult = 2**i
			model_enc_back += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * mult * 2),nn.ReLU(True)]
		...
		#seg encoder output 256xW/4xH/4
		model_enc_seg = [nn.ReflectionPad2d(1),nn.Conv2d(input_nc[2], ngf, kernel_size=3, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		n_downsampling = 1
		for i in range(n_downsampling):
			mult = 2**i
			model_enc_seg += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * mult * 2),nn.ReLU(True)]
		...
		for i in range(n_downsampling):
			...
			#model_dec_al += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),kernel_size=3, stride=2,padding=1, output_padding=1,bias=use_bias),norm_layer(int(ngf * mult / 2)),nn.ReLU(True)]
			model_dec_al += [nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * mult, int(ngf * mult / 2), 3, stride=1,padding=1),norm_layer(int(ngf * mult / 2)),nn.ReLU(True)]
		model_dec_al += [nn.ReflectionPad2d(1),nn.Conv2d(ngf, 1, kernel_size=3, padding=0),nn.Tanh()]


		# model_dec_fg1=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, int(ngf * 2), 3, stride=1,padding=1),norm_layer(int(ngf * 2)),nn.ReLU(True)]
		model_dec_fg1=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, ngf, 3, stride=1,padding=1),norm_layer(ngf),nn.ReLU(True),nn.ReflectionPad2d(1),nn.Conv2d(ngf, output_nc-1, kernel_size=3, padding=0)]

		model_dec_fg2=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, ngf, 3, stride=1,padding=1),norm_layer(ngf),nn.ReLU(True),nn.ReflectionPad2d(1),nn.Conv2d(ngf, output_nc-1, kernel_size=3, padding=0)]
...
# @yaojy
class ResnetConditionHRVeryLite_zeropad(nn.Module):
	def __init__(self, input_nc, output_nc, ngf=64, nf_part=64,norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks1=7, n_blocks2=3, padding_type='reflect'):
		assert(n_blocks1 >= 0); assert(n_blocks2 >= 0)
		super(ResnetConditionHRVeryLite_zeropad, self).__init__()
		self.input_nc = input_nc
		self.output_nc = output_nc
		self.ngf = ngf
		use_bias=True
		...
		#main encoder output 256xW/4xH/4
		# model_enc1 = [nn.ReflectionPad2d(3),nn.Conv2d(input_nc[0], ngf, kernel_size=7, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		model_enc1 = [nn.ZeroPad2d(1),nn.Conv2d(input_nc[0], ngf, kernel_size=3, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		model_enc1 += [nn.Conv2d(ngf , ngf * 2, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * 2),nn.ReLU(True)]
		# model_enc2 = [nn.Conv2d(ngf*2 , ngf * 4, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * 4),nn.ReLU(True)]
		

		#back encoder output 256xW/4xH/4
		model_enc_back = [nn.ZeroPad2d(1),nn.Conv2d(input_nc[1], ngf, kernel_size=3, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		n_downsampling = 1
		for i in range(n_downsampling):
			mult = 2**i
			model_enc_back += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * mult * 2),nn.ReLU(True)]
		...
		#seg encoder output 256xW/4xH/4
		model_enc_seg = [nn.ZeroPad2d(1),nn.Conv2d(input_nc[2], ngf, kernel_size=3, padding=0,bias=use_bias),norm_layer(ngf),nn.ReLU(True)]
		n_downsampling = 1
		for i in range(n_downsampling):
			mult = 2**i
			model_enc_seg += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3,stride=2, padding=1, bias=use_bias),norm_layer(ngf * mult * 2),nn.ReLU(True)]
		...
		for i in range(n_downsampling):
			...
			#model_dec_al += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),kernel_size=3, stride=2,padding=1, output_padding=1,bias=use_bias),norm_layer(int(ngf * mult / 2)),nn.ReLU(True)]
			model_dec_al += [nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * mult, int(ngf * mult / 2), 3, stride=1,padding=1),norm_layer(int(ngf * mult / 2)),nn.ReLU(True)]
		model_dec_al += [nn.ZeroPad2d(1),nn.Conv2d(ngf, 1, kernel_size=3, padding=0),nn.Tanh()]


		# model_dec_fg1=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, int(ngf * 2), 3, stride=1,padding=1),norm_layer(int(ngf * 2)),nn.ReLU(True)]
		model_dec_fg1=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, ngf, 3, stride=1,padding=1),norm_layer(ngf),nn.ReLU(True),nn.ZeroPad2d(1),nn.Conv2d(ngf, output_nc-1, kernel_size=3, padding=0)]

		model_dec_fg2=[nn.Upsample(scale_factor=2,mode='bilinear',align_corners = True),nn.Conv2d(ngf * 2, ngf, 3, stride=1,padding=1),norm_layer(ngf),nn.ReLU(True),nn.ZeroPad2d(1),nn.Conv2d(ngf, output_nc-1, kernel_size=3, padding=0)]
...
class ResnetBlock(nn.Module):
	...
	def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
		...
		conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=0, bias=use_bias), # padding=p
					   norm_layer(dim),
					   nn.ReLU(True)]
		if use_dropout:
